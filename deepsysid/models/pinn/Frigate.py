    ...
    def train(
        ...
        for i in range(self.epochs):
            ...
            for batch_idx, batch in enumerate(data_loader):
                ...
                batch_loss.backward()
                self.optimizer.step()
            gt = labels.detach().cpu().numpy().reshape((-1, 1))
            pred = state_pred.detach().cpu().numpy().reshape((-1, 1))
...
class FrigatePILSTMModelConfig(DynamicIdentificationModelConfig):
    inputNode: int
    hiddenNode: int
    outputNode: int
    sequence_length: int
...
class FrigatePILSTMModel(base.DynamicIdentificationModel, abc.ABC):
    CONFIG = FrigatePILSTMModelConfig
    ...
    def train(
        ...
        for i in range(self.epochs):
            ...
            for batch_idx, batch in enumerate(data_loader):
                ...
                signals = batch['x'].float().to(self.device)
                labels = batch['y'].float().to(self.device)
                state_pred = self.model.forward(signals).to(self.device)
                # print(labels.type, state_pred.type)
                MSE_r = self.loss_function(
                    state_pred, labels
                )
                delta = signals[:, :, 1].unsqueeze(dim=2)
    ...
    def simulate(
        ...
        with torch.no_grad():
            ...
            # control_in = torch.from_numpy(control).float().to(self.device)
            state = self.model.forward(control_window).to(self.device)
            y_np: NDArray[np.float64] = (
                state.cpu().detach().squeeze().unsqueeze(1).numpy().astype(np.float64)
            )
        y_np = utils.denormalize(y_np, self.state_mean, self.state_std)
